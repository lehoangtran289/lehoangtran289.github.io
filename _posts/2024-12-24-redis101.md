...
Đây là chiến lược cache thường được sử dụng nhất. Ý tưởng là bộ nhớ cache nằm ở giữa ứng dụng và data source, ứng dụng sẽ ưu tiên lấy dữ liệu từ cache, khi dữ liệu không có trong cache thì ứng dụng sẽ lấy dữ liệu data source, sau đó cập nhật lại vào cache, từ đó lần gọi sau dữ liệu đã có trong cache

Use Cases: Chiến lược này thường được sử dụng cho những kiểu dữ liệu nặng và được truy cập nhiều lần, kết quả trả về ít thay đổi.

Ưu điểm: Hệ thống sử dụng chiến lược này có khả năng phục hồi lỗi bộ nhớ cache. Nếu bộ nhớ cache bị hỏng, ứng dụng có thể truy cập trực tiếp vào data source.

Nhược điểm:

Không đảm bảo được tính nhất quán của dữ liệu, cần chiến lược cập nhật lại bộ nhớ cache phù hợp.

Khi được gọi lần đầu tiên, dữ liệu trong bộ nhớ cache không có => thừa một bước gọi vào cache => cần warm up, tải dữ liệu vào bộ nhớ cache từ trước.

### 2.2. Write through

![Untitled]({{ site.baseurl }}/images/blogs/redis101/Untitled%203.png)

Ứng dụng lấy dữ liệu tương tự như Read-Though, nhưng khi cập nhật dữ liệu thì Cache Provider sẽ ưu tiên cập nhật vào bộ nhớ cache trước, sau đó Cache Provider sẽ tự cập nhạt dữ liệu sang data source.
Use Cases: Kết hợp với Read-Though, đảm bảo tính nhất quán dữ liệu.
Ưu điểm: Đảm bảo tính nhất quán dữ liệu
Nhược điểm: Tăng độ trễ khi ghi dữ liệu
...
In order to avoid performance degradation during a background save, the redis-server process creates a child process (fork) to perform all the persistence operations in a nonblocking manner (**BGSAVE** command)
...
When AOF is enabled, every time Redis receives a command that changes the dataset, it will append that command to the AOF (Append-only File) → rebuild to correct state
